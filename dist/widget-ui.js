(function(){"use strict";const a=window.__ak;if(!a)return;const i=a.S,E=document.currentScript;function C(){try{const e=E&&E.src;if(e)return e.replace(/widget-ui\.js(\?.*)?(#.*)?$/,"accessibility-widget.css")}catch(e){}return"accessibility-widget.css"}const z='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.9 4.9l1.4 1.4M17.7 17.7l1.4 1.4M2 12h2M20 12h2M4.9 19.1l1.4-1.4M17.7 6.3l1.4-1.4"/></svg>',x='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 14.5A8.5 8.5 0 1 1 9.5 4a6.5 6.5 0 0 0 10.5 10.5z"/></svg>',M=`
...
<button class="ak-icon-btn" id="ak-theme-btn" title="Switch the panel between light and dark mode" aria-label="Toggle theme">${x}</button>
...
`,v=document.createElement("div");v.id="ak-host",document.body.appendChild(v);const t=v.attachShadow({mode:"closed"}),b=document.createElement("link");b.rel="stylesheet",b.href=C(),t.appendChild(b);const I=document.createElement("div");I.innerHTML=M,t.appendChild(I),a.setRoot(t);try{const e=window.__WIDGET_CONFIG__;e&&e.theme&&a.applyBrandTheme(e.theme)}catch(e){}if(i.logoUrl){const e=t.querySelector(".ak-logo");if(e){const n=e.innerHTML,s=e.style.background;e.innerHTML="";const l=document.createElement("img");l.src=i.logoUrl,l.alt="",l.style.cssText="width:100%;height:100%;object-fit:contain;border-radius:7px",l.onerror=()=>{e.innerHTML=n,e.style.background=s},e.style.background="none",e.appendChild(l)}}function w(){const e=t.getElementById("ak-voice-select");speechSynthesis.getVoices().filter(n=>n.lang.startsWith("en")).forEach(n=>{const s=document.createElement("option");s.value=n.name,s.textContent=n.name.replace(/\s*\(.*\)/,""),n.name===i.voice&&(s.selected=!0),e.appendChild(s)})}speechSynthesis.onvoiceschanged=w,setTimeout(w,500);const p=t.getElementById("ak-fab"),B=t.getElementById("ak-panel");let o=!1;function L(){o=!0,B.classList.add("ak-open"),p.setAttribute("aria-expanded","true")}function d(){o=!1,B.classList.remove("ak-open"),p.setAttribute("aria-expanded","false")}p.addEventListener("click",()=>{o?d():L()}),t.getElementById("ak-close-btn").addEventListener("click",d),document.addEventListener("keydown",e=>{e.altKey&&e.key==="a"&&(e.preventDefault(),o?d():L()),e.altKey&&e.key==="r"&&(e.preventDefault(),a.readPage()),e.altKey&&e.key==="s"&&(e.preventDefault(),a.stopSpeaking()),e.key==="Escape"&&o&&d()});const m=t.getElementById("ak-theme-btn");function S(){m.innerHTML=i.theme==="light"?x:z,m.title=i.theme==="light"?"Switch to dark theme":"Switch to light theme"}m.addEventListener("click",()=>{a.toggleTheme(),S()}),S(),t.querySelectorAll(".ak-tab").forEach(e=>{e.addEventListener("click",()=>{t.querySelectorAll(".ak-tab").forEach(s=>s.classList.remove("ak-active")),t.querySelectorAll(".ak-section").forEach(s=>s.classList.remove("ak-visible")),e.classList.add("ak-active");const n=t.getElementById("ak-tab-"+e.dataset.tab);n&&n.classList.add("ak-visible")})}),t.getElementById("ak-read-page").addEventListener("click",()=>a.readPage()),t.getElementById("ak-read-progress-btn").addEventListener("click",()=>a.readWithProgress()),t.getElementById("ak-read-section").addEventListener("click",()=>{d(),a.toggleSectionRead(),t.getElementById("ak-read-section").classList.toggle("ak-on")}),t.getElementById("ak-stop-btn").addEventListener("click",()=>a.stopSpeaking()),t.getElementById("ak-simplify").addEventListener("click",()=>a.simplifyPage()),t.getElementById("ak-ruler-btn").addEventListener("click",()=>{a.toggleRuler(),t.getElementById("ak-ruler-btn").classList.toggle("ak-on",i.rulerOn)}),t.getElementById("ak-links-btn").addEventListener("click",()=>{a.toggleHighlightLinks(),t.getElementById("ak-links-btn").classList.toggle("ak-on",i.highlightLinks)}),t.getElementById("ak-heads-btn").addEventListener("click",()=>{a.toggleHighlightHeadings(),t.getElementById("ak-heads-btn").classList.toggle("ak-on",i.highlightHeadings)}),t.getElementById("ak-imgs-btn").addEventListener("click",()=>{a.toggleHideImages(),t.getElementById("ak-imgs-btn").classList.toggle("ak-on",i.hideImages)}),t.getElementById("ak-anim-btn").addEventListener("click",()=>{a.toggleAnimations(),t.getElementById("ak-anim-btn").classList.toggle("ak-on",i.animationsOff)}),t.getElementById("ak-zoom-in").addEventListener("click",()=>{a.zoomIn(),c()}),t.getElementById("ak-zoom-out").addEventListener("click",()=>{a.zoomOut(),c()}),t.getElementById("ak-zoom-reset").addEventListener("click",()=>{a.zoomReset(),c()}),t.getElementById("ak-zoom-slider").addEventListener("input",e=>{a.setZoom(parseFloat(e.target.value)),c()});function c(){t.getElementById("ak-zoom-slider").value=i.zoomLevel,t.getElementById("ak-zoom-val").textContent=Math.round(i.zoomLevel*100)+"%",t.getElementById("ak-zoom-reset").textContent=Math.round(i.zoomLevel*100)+"%"}if(["none","high","dark","invert","sepia","bw"].forEach(e=>{const n=t.getElementById("ak-contrast-"+e);n&&n.addEventListener("click",()=>{a.setContrast(e),t.querySelectorAll('[id^="ak-contrast-"]').forEach(s=>s.classList.remove("ak-on")),n.classList.add("ak-on")})}),i.contrastMode&&i.contrastMode!=="none"){const e=t.getElementById("ak-contrast-"+i.contrastMode);e&&e.classList.add("ak-on")}t.getElementById("ak-focus-btn").addEventListener("click",()=>{a.toggleFocusRing(),t.getElementById("ak-focus-btn").classList.toggle("ak-on",i.focusRingOn)});const y=["normal","large","xl"],R={normal:"Large Cursor",large:"Cursor: Large",xl:"Cursor: X-Large"};let r=0;t.getElementById("ak-cursor-btn").addEventListener("click",()=>{r=(r+1)%y.length,a.setCursor(y[r]);const e=t.getElementById("ak-cursor-btn");e.classList.toggle("ak-on",r!==0),e.textContent=R[y[r]]}),t.getElementById("ak-font-up").addEventListener("click",()=>{a.fontUp(),u()}),t.getElementById("ak-font-down").addEventListener("click",()=>{a.fontDown(),u()}),t.getElementById("ak-font-reset").addEventListener("click",()=>{a.fontReset(),u()});function u(){t.getElementById("ak-font-display").textContent=i.fontSize+"%"}t.getElementById("ak-dyslexic-btn").addEventListener("click",()=>{a.toggleDyslexic(),t.getElementById("ak-dyslexic-btn").textContent=i.dyslexicOn?"Dyslexia Font: ON":"Dyslexia-Friendly Font",t.getElementById("ak-dyslexic-btn").classList.toggle("ak-on",i.dyslexicOn)}),t.getElementById("ak-voice-select").addEventListener("change",e=>{i.voice=e.target.value,a.save()}),t.querySelectorAll("[data-spacing]").forEach(e=>{e.addEventListener("click",()=>{const[n,s]=e.dataset.spacing.split("-"),l={line:"lineSpacing",letter:"letterSpacing"};l[n]&&(i[l[n]]=s,t.querySelectorAll(`[data-spacing^="${n}-"]`).forEach(k=>k.classList.remove("ak-active")),e.classList.add("ak-active"),a.applySpacing())})}),t.getElementById("ak-rate").addEventListener("input",e=>{i.rate=parseFloat(e.target.value),t.getElementById("ak-rate-val").textContent=i.rate.toFixed(1)+"\xD7",a.save()}),t.getElementById("ak-pitch").addEventListener("input",e=>{i.pitch=parseFloat(e.target.value),t.getElementById("ak-pitch-val").textContent=i.pitch.toFixed(1),a.save()}),t.getElementById("ak-vol").addEventListener("input",e=>{i.volume=parseFloat(e.target.value),t.getElementById("ak-vol-val").textContent=Math.round(i.volume*100)+"%",a.save()}),t.getElementById("ak-test-voice").addEventListener("click",()=>{a.speak("Hello! This is how I sound. Accessibility made easy with Accessibility Assistant.")}),t.getElementById("ak-summarize-btn").addEventListener("click",async()=>{const e=t.getElementById("ak-summary-box");e.textContent="Summarizing\u2026",e.classList.add("ak-visible");const n=await a.aiSummarize();e.textContent=n||"Could not summarize. Check your API key."}),t.getElementById("ak-simplify-text-btn").addEventListener("click",async()=>{var e,n;const s=(n=(e=window.getSelection())==null?void 0:e.toString())==null?void 0:n.trim();if(!s){a.toast("Select some text first");return}const l=t.getElementById("ak-simplified-box");l.textContent="Simplifying\u2026",l.classList.add("ak-visible");const k=await a.aiSimplifyText(s);l.textContent=k||"Error simplifying text.",k&&a.speak(k)});const h=t.getElementById("ak-chat-msgs"),f=t.getElementById("ak-chat-input");function g(e,n){const s=document.createElement("div");return s.className="ak-chat-bubble ak-"+n,s.textContent=e,h.appendChild(s),h.scrollTop=h.scrollHeight,s}async function A(){const e=f.value.trim();if(!e)return;f.value="",g(e,"user");const n=g("Thinking\u2026","thinking"),s=await a.aiAnswer(e,i.chatHistory);if(n.remove(),s.error){g("Error: "+s.error,"bot");return}g(s.text,"bot"),i.chatHistory.push({role:"user",content:e},{role:"assistant",content:s.text}),i.chatHistory.length>12&&i.chatHistory.splice(0,2)}t.getElementById("ak-chat-send").addEventListener("click",A),f.addEventListener("keydown",e=>{e.key==="Enter"&&A()}),t.getElementById("ak-api-save").addEventListener("click",()=>{const e=t.getElementById("ak-api-key").value.trim();i.mistralKey=e,a.save();const n=t.getElementById("ak-api-dot"),s=t.getElementById("ak-api-text");e?(n.className="ak-dot ak-ok",s.textContent="API key saved"):(n.className="ak-dot ak-err",s.textContent="No key set"),a.toast("API key saved")}),i.mistralKey&&(t.getElementById("ak-api-key").value=i.mistralKey,t.getElementById("ak-api-dot").className="ak-dot ak-ok",t.getElementById("ak-api-text").textContent="API key set"),t.getElementById("ak-reset-btn").addEventListener("click",()=>{confirm("Reset all accessibility settings?")&&(localStorage.removeItem("__ak_state"),location.reload())}),t.getElementById("ak-rate").value=i.rate||1,t.getElementById("ak-rate-val").textContent=(i.rate||1).toFixed(1)+"\xD7",t.getElementById("ak-pitch").value=i.pitch||1,t.getElementById("ak-pitch-val").textContent=(i.pitch||1).toFixed(1),t.getElementById("ak-vol").value=i.volume||1,t.getElementById("ak-vol-val").textContent=Math.round((i.volume||1)*100)+"%",c(),u(),a.applyAll()})();
